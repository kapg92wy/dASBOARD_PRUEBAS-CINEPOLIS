...
async function updateBadge() {
  ...
  try {
    ...
    let incs = await DB.getIncidencias(filtros);
    // El técnico solo cuenta las incidencias que tiene asignadas
    if (currentUser.rol === 'tecnico') incs = incs.filter(r => r.tecnico_id === currentUser.id);
    const count = incs.filter(r => r.estado !== 'Resuelta' && r.estado !== 'Cerrada').length;
