      ...
      + '<tbody>' + users.map(u => `
          <tr>
            ...
            <td style="color:var(--text2);font-size:11px;">${u.email
              ? u.email
              : '<span style="color:var(--text3);">— sin correo —</span>'}${u.telefono ? '<br><span style="color:var(--cyan);">📱 '+u.telefono+'</span>' : ''}</td>
...
async function saveUser() {
  ...
  try {
    const ts = nowISO();
    if (editingUserId) {
      const cambios = { nombre, username, rol, email, telefono, updated_at: ts };
      ...
      await DB.crearUsuario({
        id: newId('u'), username, password: pass, rol, nombre, email, telefono,
