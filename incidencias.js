...
async function recargarMisAsignadas() {
  try {
    _misAsig = await DB.getIncidencias({ tecnico_id: currentUser.id });
    filtrarMisAsignadas();
  } catch(err) {
...
async function renderIncidenciasCines(container) {
  container.innerHTML =
    ...
    + '<tbody id="tbCinesLista">' + loadingRow(10) + '</tbody></table></div></div>';
  await recargarCines();
}
...
async function openModal(id) {
  ...
  try {
    ...
    if (['mantenimiento','admin'].includes(currentUser.rol)) {
      try { _tecnicos = await DB.getTecnicos(); window._tecnicosCache = _tecnicos; }
      catch(e) { console.warn('[openModal] getTecnicos falló:', e.message); _tecnicos = []; }
    }
    ...
    document.getElementById('modalContent').innerHTML = `
      ...
      ${r.tecnico_nombre ? `<div class="detail-row"><span class="detail-key">Técnico asignado</span><span class="detail-val" style="color:var(--gold);font-weight:600;">🔧 ${r.tecnico_nombre}${r.tecnico_telefono ? ' · ' + r.tecnico_telefono : ''}</span></div>` : ''}
      ...
      ${canEditManto ? `
        <div style="margin-top:20px;border-top:1px solid var(--border);padding-top:16px;background:var(--bg2);padding:15px;border-radius:8px;">
          ...
          ${canAsignar ? `
          <div class="detail-key" style="margin:14px 0 8px;color:var(--gold);">Asignar Técnico de Campo</div>
          <select class="status-select" id="modalTecnico">
            <option value="">— Sin asignar —</option>
            ${_tecnicos.map(t => `<option value="${t.id}" ${r.tecnico_id===t.id?'selected':''}>${t.nombre}${t.telefono ? ' · ' + t.telefono : ''}</option>`).join('')}
          </select>
          <button class="save-status-btn" id="saveTecnicoBtn" onclick="asignarTecnico()" style="margin-top:10px;">Guardar Asignación</button>
          ${_tecnicos.length ? '' : '<div style="font-size:11px;color:var(--text3);margin-top:6px;">⚠️ No hay técnicos registrados. Créalos en la pestaña Usuarios con rol "Técnico (Campo)".</div>'}` : ''}
...
async function saveStatus() {
  ...
  try {
    ...
    if (selTec) {
      ...
      if (tid !== (r.tecnico_id || '')) {
        const tecnicos = window._tecnicosCache || [];
        const t = tid ? tecnicos.find(x => x.id === tid) : null;
        const tnombre = t ? t.nombre : '';
        cambios.tecnico_id       = tid;
        cambios.tecnico_nombre   = tnombre;
        cambios.tecnico_telefono = t ? (t.telefono || '') : '';
    ...
    const extra = (cambios.tecnico_id !== undefined)
      ? { tecnico_id: cambios.tecnico_id, tecnico_nombre: cambios.tecnico_nombre,
          tecnico_telefono: cambios.tecnico_telefono, fecha_asignacion: cambios.fecha_asignacion }
      : {};
...
/* ── Asignar técnico sin tocar el estado (solo gerente/admin) ── */
async function asignarTecnico() {
  if (!requireRole('mantenimiento','admin')) return;
  ...
  try {
    ...
    let cambios = { tecnico_id: '', tecnico_nombre: '', tecnico_telefono: '', fecha_asignacion: null, push_enviado: null };
    ...
    if (tecnicoId) {
      ...
      if (t) {
        cambios = { tecnico_id: t.id, tecnico_nombre: t.nombre, tecnico_telefono: t.telefono || '',
                    fecha_asignacion: nowISO(), push_enviado: 0 };  // 0 = el script le mandará el aviso
        nombreTec = t.nombre;
      }
    }
    ...
    await DB.escribirLog({ id: newId('log'), incidencia_id: editingIncId,
      usuario_id: currentUser.id, nombre_usuario: currentUser.nombre,
      accion: 'asignacion', estado_anterior: r.tecnico_nombre || '(sin asignar)',
      estado_nuevo: nombreTec,
      nota: `Asignada a ${nombreTec} por ${currentUser.nombre}`, created_at: nowISO() });
